...
- 本文には **選択テキスト＋元ページのURLを併記**して保存（メモ単体で出典が分かるように）。
  - 初期表示の内容＝`選択テキスト` + 空行 + `元ページURL`（編集可）。
  - ※URLは**素のページURL**（v1.2.0で試したテキストフラグメント `#:~:text=` 付き該当箇所リンクは、%エンコードで激しく見づらいため v1.2.1 で撤回）。
- 保存：`learningStore.add(uid, { dateKey, title, content, url, sortOrder })`。
  - これは StudyTracker の「特急メモ」と同一（`notionPageId` なし＝インボックス扱い）。
  - `content` に出典URLを含み、`url` フィールドにも同じページURLを保存。
