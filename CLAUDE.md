...
## 🏠 NotionPlus の入口はホーム画面（v1.0.321〜）

`/notion-plus` は**ダッシュボード**（続きから／最近開いたページ／お気に入り）。
以前のように `lastViewedNotionPageId` へ `router.replace` して**自動で飛ばさない**（本人指示 2026-09-11）。

- 🔥 **大タブは NotionPlus の「開いていたページ」を覚えない**（`TopTabs.tsx` の `rememberable` が `null` を返す）。
  覚えているとタブを押してもホームへ辿り着けない。前回の続きはホームの「続きから」が受け持つ。
  ⚠ **`readStore()` の読み出し側でも捨てている**＝v1.0.320 以前の記憶が localStorage に残るため。
  書き込みを止めるだけでは古い記憶で直リンクし続ける（2026-09-11 に実際に踏んだ）。
- 閲覧履歴 `settingsStore.recentNotionPageIds` は **localStorage にだけ持つ**。
  Firestore に書かない（無料枠の読み書きを増やさないため）。
- ⚠ **ホームにページ一覧を置かない。** 左のサイドバーに常時出ていて重複する。

## 📚 サイドバーに出さないのは「親を持つブック」だけ（v1.0.323〜）

`Sidebar.tsx` の `PageTreeEntry` は `page.type === 'book' && page.parentId` のときだけ `return null`。
🔥 **`&& page.parentId` を外さない。** 一律で隠すと**最上位のブックが行方不明になる**
（親の本文にリンクが載らないので、サイドバーが唯一の入口。2026-09-11「訴求マスタ」で実際に起きた。
最上位ノートを右クリック →「ブックに変換」した瞬間に消える）。
⚠ **この判定は hooks より後に置く。** 変換の瞬間に `type` が page→book へ変わるため、
hooks の前で `return null` すると hooks の数が変わって React が落ちる。

## 🔁 復習は「今日の復習」パネルに一本化（v1.0.321〜）

学習リストのダッシュボードで、**左の「今日の登録」に復習を出さない。**
v1.0.320 まで左下に赤い「昨日の学習」パネルがあったが、右の「翌日」と同じ段階なのに
名前の付け方（学習日 / 復習の間隔）が違い、**違いが本人にも分からなかった**ので撤去した。
**復活させない。** 遅れは学習日の見出しに `◯日遅れ` で出す（`StudiedOnHeader` / `lateDaysOf`）。
...
雛形＝`C:\dev\Private\Skill\学び\NP整理\core\cdp-verify-rootmenu3.mjs` / `_shot-page.mjs`（スクショ）。
