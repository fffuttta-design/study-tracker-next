...
- ⚡ 特急メモ（未消化）パネル
  - 折りたたみ可能なカードリスト
  - 各カードに：「✏️ 編集」「🗑 削除」ボタン（消化はエディタの「/特急メモ」で行う。AI整理は廃止）
  - 登録日表示
- 消化済み学習アイテム（時間帯グループ）
  - `HH:00～` でグループ化
...
### 4.2 消化（エディタの「/特急メモ」でカーソル位置に挿入）

> 消化＝特急メモを「正式なメモ」へ移行する。**挿入位置はユーザー指定（カーソル位置）**。AI は使わない。要件定義の正本は `特急メモ消化要件定義.md`。

- NotionPlus ページのエディタで **スラッシュ「/特急メモ」**（`SlashCommand.openMemoPicker`）→ **特急メモ挿入ピッカー**がカーソル位置近くに開く（`NotionEditor.tsx`）。
- ピッカー：検索＋**未消化の特急メモ一覧**（`notionPageId` 未設定の `learningItem`・新しい順・40件）。
- メモを1件クリックで **消化実行**（`insertMemoAtCursor`）：
  1. **カーソル位置へ挿入**（見出し3［タイトル］＋段落［本文・改行で段落分割］を `insertContent`）
  2. **学習アイテムを1件作成**（`notionPageId`＝開いているページ・content ベース）→ 復習ルーティンへ
  3. **元の特急メモを削除**
- ※旧「AI整理モーダル」と一時版「カード側の消化ダイアログ（末尾追記）」は廃止（`/api/ai-triage` ルートは当面残置・未使用）。
...
| 日付 | バージョン | 内容 |
|---|---|---|
| 2026-06-26 | （次回配信） | 改修：**消化の挿入位置を「カーソル位置（ユーザー指定）」に変更**し、動線を**エディタ側「/特急メモ」**へ移行（要件定義更新）。NotionPlus ページで `/特急メモ`（`openMemoPicker`）→ 未消化メモのピッカー → 選んだメモを**カーソル位置に挿入**（`insertMemoAtCursor`＝見出し3＋本文段落）＋学習アイテム作成（開いているページ紐づけ）＋元メモ削除。直前の v1.0.254「カード側 DigestDialog（末尾追記）」は撤去（DigestDialog/appendMemoToPageContent/buildPagePath/PageIcon 削除・インボックスカードの「消化する」ボタンも撤去）。NotionEditor.tsx / learning/page.tsx |
