...
| 日付 | バージョン | 内容 |
|---|---|---|
| 2026-06-30 | （次回配信） | 追加：**グローバルホットキー `Ctrl+Shift+S` でどのアプリからでも特急メモを追加**（Discord 等の上に小窓を出す）。デスクトップ常駐アプリに `globalShortcut` を登録し、押すと枠なし・最前面・タスクバー非表示のクイック入力ポップアップ（既存の Web ルート `/clip` を流用＝メイン窓とログイン共有・特急メモに1件記録して自動で閉じる）を表示。**起動時にクリップボードの内容を自動で流し込む**（`clipboard.readText()` → 先頭の非空行＝タイトル／残り＝本文に分割して `?title=&content=` で `/clip` へ渡す）。Escape／✕で閉じる。トレイメニューにも「特急メモを追加 (Ctrl+Shift+S)」を追加。`/clip` はタイトル未入力時に本文1行目をタイトルへ流用するよう改善。electron/main.js（showQuickCapture）/ apps/web/src/app/clip/page.tsx |
