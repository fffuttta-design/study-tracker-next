...
- 🔥 **ウィンドウの排他制御＝同じ画面を2つの窓に出さない（v1.0.315〜）。**
  ...
  **学習リストが2窓に並ぶ**（2026-09-08 本人指摘。しかも古い方は開いた日のまま止まるので、
  ...
  - ⚠ **画面側（TopTabs）だけでなく、メインプロセスにも網を張ってある**（`guardWindowSection`）。リンクや `router.push` で担当外へ流れても、担当の窓へ渡して自分は持ち場へ戻る。
  - ⚠ **戻すのに `history.back()` は使わない。** ログイン直後の自動遷移のように「戻ると同じ所へまた飛ぶ」経路があり、往復し続けるため。素直に持ち場を読み直す。
  - ⚠ **ログイン後の行き先は窓ごとに変える**（`login/page.tsx`）。NotionPlus窓まで `/learning` へ飛ばすと排他制御に弾かれて往復する。
