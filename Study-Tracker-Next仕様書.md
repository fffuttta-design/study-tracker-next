...
- 🔥 **ウィンドウの排他制御＝同じ画面を2つの窓に出さない（v1.0.315〜）。**
  大タブはどの窓にも出ているので、放っておくと「NotionPlus窓で"学習リスト"タブを押す」だけで
  **学習リストが2窓に並ぶ**（2026-09-08 本人指摘。しかも古い方は開いた日のまま止まるので、
  昨日の日付の窓が残って混乱する）。∴ **画面ごとに担当の窓を決める**。

  | 窓 | 担当する画面 |
  |---|---|
  | メイン窓（`main`） | 学習リスト／覚えるリスト／設定／カテゴリ／要改修 |
  | NotionPlus窓（`notion`） | NotionPlus |
  | ノート別窓（`note`） | 開いたノート1枚（**大タブはどれを押してもよその窓へ渡す**） |
  | 特急メモ小窓（`quick`） | 大タブが無いので対象外 |

  - 担当外のタブを押しても**その窓では開かず、担当の窓を前に出す**（すでにその画面を開いていれば読み直さない＝編集中を壊さない）。Ctrl+Tab も同じ。
  - 窓の種別は Electron 側が `webPreferences.additionalArguments`（`--st-window=<種別>`）で名乗らせ、`preload.cjs` が `window.electronAPI.windowKind` として渡す。渡す先は IPC `open-section` → `showSection()`。
  - 🔥 **効かせるのは「大タブ」と「ディープリンク」だけ。本文中のリンクは今までどおりページ遷移**（v1.0.317〜・本人指示）。
    合言葉は **「タブ＝窓の切り替え／リンク＝ページ遷移」**。
    ⚠ v1.0.315 では全てのナビゲーションをメインプロセスで見張っていた（`guardWindowSection`）が、
    それだと学習リストの「📖 ノートを開く」を押しただけで **NotionPlus窓が新しく立ち上がって**しまい、
    かえって鬱陶しくなった（2026-09-08 本人指摘）。**この見張りは撤去した。復活させない。**
  - ⚠ **ログイン後の行き先は窓ごとに変える**（`login/page.tsx`）。NotionPlus窓まで `/learning` へ飛ばすと持ち場から外れる。
  - 🔥 **大タブに画面を足すときは、必ず担当の窓も決める**（`electron/main.js` の `sectionOwner()` と `TopTabs.tsx` の `ownerOf()` の両方）。決め忘れると既定でメイン窓扱いになる。
