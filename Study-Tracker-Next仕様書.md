...
- 「確定」で：①エディタの現在内容を `contentGetterRef`（`NotionEditor` に追加した即時ゲッター）で取得 → ②既存ページ末尾へ追記 or 新規ページ作成して書き込み（`appendDocToPageContent`）③`learningItem` を1件作成（content＝整えた内容のプレーンテキスト・`notionPageId`＝追記先）→ 復習へ ④元の特急メモ削除。
