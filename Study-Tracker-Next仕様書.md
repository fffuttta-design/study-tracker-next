...
- ⚡ 特急メモ（未消化）パネル
  - 折りたたみ可能なカードリスト
  - 各カードに：「✏️ 編集」「消化する →」「🗑 削除」ボタン（※AI整理は廃止）
  - 登録日表示
- 消化済み学習アイテム（時間帯グループ）
  - `HH:00～` でグループ化
...
### 4.2 消化ダイアログ（DigestDialog・「消化する」からトリガー）

> 消化＝特急メモを「正式なメモ」へ移行する。AI は使わないシンプル動線。要件定義の正本は `特急メモ消化要件定義.md`。

- 「消化する →」を押すと**消化ダイアログ**が即開く（AI ロード無し）。
- ダイアログ内容：
  - **タイトル・本文の手直し欄**（消化前に整える。AI 整形はしない）
  - 「**＋ 新規ページを作成して消化**」（新ページ作成・タイトル＝メモのタイトル／空なら「無題メモ」・icon 📝）
  - **既存ページ検索＋一覧**（通常ノートのみ：`workspace`／DB／ブック除外・更新日時の新しい順・40件）
- 既存ページ or 新規を選ぶと **消化実行**：
  1. 対象ページの**本文末尾に追記**（`appendMemoToPageContent`＝見出し3［タイトル］＋段落［本文・改行で段落分割］）
  2. **学習アイテムを1件作成**（`notionPageId`＝対象ページ・content ベース）→ 復習ルーティンへ
  3. **元の特急メモを削除**
- ※旧「AI整理モーダル」は廃止（`/api/ai-triage` ルートは当面残置・未使用）。
...
| 日付 | バージョン | 内容 |
|---|---|---|
| 2026-06-26 | （次回配信） | 改修：**特急メモの「消化」をシンプル版に作り替え**（要件定義＝`特急メモ消化要件定義.md`）。旧 AI整理モーダルを廃止し、「消化する」で開く `DigestDialog`（タイトル/本文の手直し欄＋「新規ページ作成して消化」＋既存ページ検索）に置換。消化＝①対象ページ本文末尾へ追記（見出し3＋本文段落・`appendMemoToPageContent`）②学習アイテム作成で復習ルーティン登録③元メモ削除。AI整理ボタン/モーダル（AiTriageModal）と関連ヘルパを撤去（`/api/ai-triage` は未使用で残置）。learning/page.tsx |
