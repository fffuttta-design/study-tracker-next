...
- **表示デザイン（2026-06-16・コールアウト/カンバン化）**：枠線の表をやめ、**Trello風カンバン＋コールアウト**に。列＝淡色の角丸リスト（既定グレー・`color`で6色変更可）、各リンク＝白い角丸カード（ページ絵文字＋ページ名・装飾アイコンは無し）、下部に「＋カードを追加」、リストは横並び＋折り返し。大見出しはカード群の上の小ラベル。`PtColumn.color?` を追加（既存データは自動でこの表示に・移行不要）。
