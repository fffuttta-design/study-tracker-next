...
- **キーボードでタブ切替（v1.0.302〜）**：**Ctrl+Tab** で「学習リスト → NotionPlus → 覚えるリスト」を順送り、**Ctrl+Shift+Tab** で逆送り（端まで行ったら反対の端へ回る）。クリックと同じ扱いで、離れる瞬間に居場所を記録し、戻り先も前回の続き。設定画面などタブの外にいるときは先頭（学習リスト）へ。⚠ **デスクトップ（Electron）版だけ**で効く＝ブラウザでは Ctrl+Tab がブラウザ自身のタブ切替に予約されていて `preventDefault` が効かず、アプリ側も動くと二重に切り替わるため `window.electronAPI` があるときだけ購読する。キー処理は `capture` フェーズで拾い、TipTap などページ側より先に取る。
- 🔥 **大タブは「今いる窓」で開く。窓は増やさない（v1.0.318〜・本人指示）。**
  どのタブを押しても、その窓がそのままそのページへ移動する。**新しい窓は立ち上がらない。**
  - ⚠ v1.0.315〜317 は逆で、画面ごとに担当の窓（`main`／`notion`／`note`）を決め、
    担当外のタブを押すと**その窓を前に出す＝窓がもう1つ増える**作りだった。
    しかし「NotionPlusのタブを押しただけで新しい窓になる」のが鬱陶しく、撤去した（2026-09-10 本人指摘）。
    `TopTabs.tsx` の `ownerOf()`／`isForeign()`／`handoff()` はこの版で削除。**復活させない。**
  - 元々この排他制御を入れた動機（＝同じ画面が2窓に並び、**古い方が昨日の日付のまま止まる**）は、
    **`useToday()` が別に解決している**（→ §4.1）。∴ 窓を分ける必要はもう無い。
  - 🔥 **窓を切り替えるのはディープリンク（`studytracker://`）とトレイだけ**＝`electron/main.js` の
    `sectionOwner()` → `showSection()`。`/notion-plus` を指すディープリンクは**専用アイコンの NotionPlus窓**を
    前に出す（`/studytracker-seiri` スキルの `open-in-app.mjs` がこれを使う）。ここは残してある。
  - 窓の種別（`--st-window=<種別>` → `window.electronAPI.windowKind`）はログイン後の行き先の判定
    （`login/page.tsx`）に今も使う。NotionPlus窓を `/learning` へ飛ばさないため。
  - IPC `open-section` と `showSection()` はディープリンク用に残っているが、**画面側からはもう呼ばない。**
...
**日付ヘッダー**（常時表示）
- 🔥 **常に「今日」を映す。前後の日へは送れない（v1.0.315〜）。**
  以前は ‹ › で前日／翌日へ移動できたが撤去した（2026-09-08 本人指摘）。理由は3つ：
  ① 登録は常に今日に入る（`dateKey: localDateKey()`）ので、前日を出しても**書けず見るだけ**だった。
  ② なのに見出しは「今日の登録」「本日の学習」のままで、**別の日を今日として読み違える**。
  ③ 過去を見る用途は「全学習リスト」タブが日付ごとに並べて見せるので足りている。
- 🔥 **日をまたぐと勝手に切り替わる**（`hooks/useToday.ts`）。このアプリはトレイ常駐で何日も開きっぱなしになるため、起動時に1回 `new Date()` するだけだと**翌日も昨日の日付のまま止まる**（2026-09-08、前日から開いていた窓が「9月7日」のまま残っていた）。0時ちょうどのタイマーに加えて、**窓のフォーカス復帰・表示復帰**でも見直す（スリープ中の取りこぼし対策）。「昨日の学習」パネルも同じ今日から数える。
- 日付を `text-3xl` で大きく表示（今日バッジ付き）
...
- ⚡ 特急メモ（未消化）パネル
  - 折りたたみ可能なカードリスト
  - 各カードに：「✏️ 編集」「消化する →」「🗑 削除」ボタン（消化は消化モーダルで行う。AI整理は廃止）
  - 登録日表示
- 消化済み学習アイテム（時間帯グループ）
  - `HH:00～` でグループ化
- 「＋ 追加」「⚡ 特急」ボタン
...
### 4.2 消化モーダル（DigestDialog・「消化する」からトリガー）

> 消化＝特急メモを「正式なメモ」へ移行する。要件定義の正本は `特急メモ消化要件定義.md`。

- ⚡ 特急メモの「**消化する →**」→ **消化モーダル**（`DigestDialog`・`learning/page.tsx`）。
- モーダルの中身（**実際の対象ノートを開いてカーソル位置に挿入**する方式）：
  - 上部に**追記先トグル**：「既存ページ」（検索＋横並びチップ・通常ノートのみ）/「新規ページ」（タイトル入力）。
  - 追記先を選ぶと、**対象ノートが本物の編集画面（`NotionEditor` 動的import流用・`hideTitle`・`stickyToolbar`）でそのまま開く**（既存＝実内容／新規＝空・`key` で切替時リロード・`notionPageId` 無し＝`/特急メモ` は挿入のみ）。
  - 入れたい場所に**カーソルを置いて**フッター「**📥 カーソル位置にメモを挿入**」→ その場に特急メモ（見出し3＋本文）を挿入（`NotionEditor.insertAtCursorRef`）。続けて整形可。挿入せず確定したら末尾追記（`appendBlocksToDoc`）。
...
**サイドバーの「📄 ページ一覧」**
- **最上位（ルート）のページだけを平らに並べる**（子ページはツリーに出さない。親の本文のリンクから辿る）。
- 🔥 **ブックは「親を持つものだけ」隠す**（`Sidebar.tsx` の `PageTreeEntry`・v1.0.323〜）。
  親の本文にブックへのリンクが載るので、ツリーにも出すと二重になるため隠している。
  しかし**最上位のブックには載せてくれる親の本文が無い**ので、隠すとサイドバーから消えて行き場が無くなる。
  ⚠ 2026-09-11 に「訴求マスタ」が実際にこの状態だった。**最上位のノートを右クリック →「ブックに変換」
  （`convertToBook`）すると、その瞬間にサイドバーから消える**のが原因。データは壊れていない。
  ⚠ この判定は**hooks より後に置く**こと。変換の瞬間に `type` が page→book へ変わるため、
  hooks の前で `return null` すると hooks の数が変わって React が落ちる。
...
- **完了チェックボックスだけ**：押すと `status` が `todo` ⇄ `done` に切り替わる。完了は打ち消し線＋薄字
- セクションは **未完了 / 完了** の2つ（完了は下に沈む）
- フィルター：**全て / 未完了 / 完了**
- 追加・編集モーダル：**タイトルとメモだけ**
- **ドラッグ＆ドロップで並び替え**：行にマウスを乗せると左端に掴む所（⠿）が出る。掴んで動かすと、狙った行の**上か下に青い線**が出てその位置へ入る。離すと `order` を 0 から振り直して保存。
  - 🔥 **掴む所を押している間だけ `draggable` にする**（行を常時 draggable にすると、文字を選ぼうとしただけでドラッグが始まる）
  - **未完了と完了は跨げない**（別セクション同士のドロップは無視する）
- メモがあるカードはクリックで展開
- 編集 / 削除

🔥 **廃止したもの**：ステータス3段階（学習中）／カテゴリ／優先度。
`Goal` 型には `category` / `priority` / `status:'learning'` が残っているが**既存データ用の名残で、画面もフォームも使わない**（`@deprecated` 明記）。
`status` が `'learning'` の古いデータは**未完了として扱う**ので移行作業は不要。
...
- **通知バーの小アイコン** = `res/drawable/ic_notification.xml`（ノート＋チェックの白シルエット）、
  差し色 = `@color/notification_color`（#F59E0B）。どちらもマニフェストの
  `com.google.firebase.messaging.default_notification_icon` / `..._color` で指定する。
  🔥 **指定しないとランチャーアイコンが使われ、Androidがアルファだけを見て塗りつぶすので
     「灰色の四角」になる**（＝既定のままに見える）。指す先は必ず**白のシルエット＋透明**にする。
...
| 日付 | バージョン | 内容 |
|---|---|---|
| 2026-09-11 | v1.0.323 | 修正：**最上位のブックがサイドバーから消える**のを解消（→ §4.5・本人指摘）。`PageTreeEntry` が `type === 'book'` を**一律で**隠していた。親を持つブックは親の本文にリンクが載るので隠して正しいが、**最上位のブックには親の本文が無い**ため、サイドバーから辿れなくなっていた（「訴求マスタ」が該当。最上位ノートを「ブックに変換」した瞬間に消える）。`&& page.parentId` を足して**親を持つブックだけ**隠すようにした。あわせて判定を hooks より後ろへ移動（変換の瞬間に type が変わり、hooks の前で return すると hooks の数が変わって落ちるため）。<br>⚠ **データは壊れていない**（全447ページを棚卸し済み：親が消えた迷子ページ 0件）。ページを移動させる修正はしていない。 |
...
| 2026-09-08 | v1.0.315 | 改善：**ウィンドウの排他制御＝同じ画面が2つの窓に並ばないようにした**（→ §4.0）。大タブはどの窓にも出ているので、NotionPlus窓で「学習リスト」タブを押すだけで学習リストが2窓に並び、しかも古い方は開いた日のまま止まるので**昨日の日付の窓が残って**混乱していた（本人指摘）。画面ごとに担当の窓（main／notion／note）を決め、**担当外のタブはその窓では開かず、担当の窓を前に出す**形にした。窓の種別は `additionalArguments`（`--st-window=`）→ `preload` → `window.electronAPI.windowKind`、移動は IPC `open-section` → `showSection()`。画面側（`TopTabs`）だけでなく**メインプロセスにも網**（`guardWindowSection`）を張り、リンクや `router.push` で流れても持ち場へ戻す。あわせて**ログイン後の行き先を窓ごとに変えた**（NotionPlus窓を `/learning` へ飛ばすと排他制御に弾かれて往復するため）。<br>改善：**学習リストの日付を前後に送る機能（‹ › と「今日へ」）を撤去し、常に今日を映すようにした**（→ §4.1）。登録は常に今日に入るので前日を出しても見るだけで、しかも見出しは「今日の登録」のままだった。過去は「全学習リスト」タブで足りる。<br>バグ修正：**窓を開きっぱなしにすると翌日も昨日の日付のまま止まる**問題を解消（`hooks/useToday.ts` 新規）。0時のタイマーに加えて窓のフォーカス／表示復帰でも日付を見直す（スリープ中の取りこぼし対策）。 |
...
| 2026-08-20 | v1.0.287 | 新機能：**ノート詳細ヘッダーに「🔄 更新」ボタン**（`notion-plus/[id]/page.tsx`・履歴ボタンの隣）。押すと `editorKey` をインクリメントしてエディタを再マウントし、ストア（onSnapshotでライブ購読済み＝`page.content`は最新）の内容で開き直す。別端末や管理SDK/Claude Codeから本文を書き換えても、開きっぱなしのエディタはマウント時内容を保持し続ける仕様のため「直したのに画面が変わらない」が起きる。それをアプリ全体の再起動なしに自己解消できる。押下後1.4秒「✓ 最新に更新」を表示。状態＝`refreshed`。 |
| 2026-08-13 | v1.0.275 | 修正：**Android配信ビルドの安定化**（`scripts/build-and-sync.mjs`）。従来は `assembleDebug` で全4ABI（armeabi-v7a/arm64-v8a/x86/x86_64）を並列C++コンパイルし、この16GB機ではメモリ枯渇でgradleデーモンがOSにkillされ「daemon disappeared」で失敗していた。ビルド前に古いjavaデーモンを掃除し、**arm64-v8a単一ABI＋`--max-workers=2`** に絞って通すよう変更（現代のスマホは全てarm64なので実用差なし。x86エミュレータ向けは非対象になる点のみ留意）。build 295のAPKを手動ビルドしてRelease `v1.0.275` へ追加、`version.json` に `downloadUrl` を書きOTA復旧済み。 |
...
| 2026-08-12 | v1.0.272 | 新機能：**NotionPlus を専用アイコンから独立起動できるように**（母体アプリ StudyTracker と同じデータ・同じ更新の仕組みのまま、見た目は別アプリ）。<br>**Windows**：デスクトップ／スタートメニューに「NotionPlus」アイコン（紫のページ＋バッジ・`build/icon-notionplus.ico`）を追加。押すと `学習トラッカー.exe --notionplus` が起動し、**NotionPlus 専用ウィンドウ**（`/notion-plus`）を開く。タスクバーでも `setAppDetails`(appId=`com.studytracker.notionplus`)で学習トラッカーと別グループ・別アイコン表示。ショートカットは起動時に自己生成/更新（`ensureNotionPlusShortcuts`＝`shell.writeShortcutLink`。インストーラ版・開発機robocopy版どちらでも／更新でパスが変わっても追随）。`--notionplus` はコールドスタート(`whenReady`)・多重起動(`second-instance`)の両方で判定。共通の `handleWindowOpen` を切り出しメイン窓と同じログイン挙動に。electron/main.js / package.json（extraResources に icon-notionplus.ico）/ scripts/generate-notionplus-icon.mjs（新規・アイコン生成）<br>**Android**：ランチャーに「NotionPlus」専用アイコンを追加（`AndroidManifest.xml` に activity-alias `.NotionPlusAlias`＝MainActivity を別名で起動・独自 label/icon）。タップすると intent の component 名から自作ネイティブモジュール `LaunchRouteModule`/`LaunchRouteHolder`（`SharedText` と同型・外部ライブラリ不使用）が「NotionPlus」を判定し、`src/services/launchRoute.ts`＋App.tsx の `LaunchRouter` が**起動時に NotionPlus タブへ遷移**（コールドスタート=`getInitialLaunchRoute`／起動中の再タップ=`onNewIntent`→`LaunchRouteReceived`。ナビゲータ準備＆ログイン完了を待って遷移）。mipmap-* に `ic_launcher_np(.png/_round.png)` を全密度生成。android（MainActivity.kt / SharedTextPackage.kt / AndroidManifest.xml / strings.xml / mipmap-*）/ apps/mobile（App.tsx / services/launchRoute.ts） |
...
| 2026-06-30 | （次回配信） | 追加（Android）：**他アプリのテキストを「共有」「テキスト選択メニュー」から特急メモに送れる**ように。AndroidManifest に MainActivity の intent-filter を2つ追加（`ACTION_SEND` text/plain＝共有メニュー、`ACTION_PROCESS_TEXT` text/plain＝文字選択メニュー）。受け取り用に自作ネイティブモジュール `SharedTextModule`／`SharedTextPackage`（外部ライブラリ不使用）を追加：コールドスタートは `getInitialSharedText()`、起動中は `MainActivity.onNewIntent`→`SharedTextHolder`→`SharedTextReceived` イベントで JS へ。JS 側は `src/services/sharedText.ts` ＋ App.tsx の `SharedTextCapture` で受信し、**先頭の非空行＝タイトル／残り＝本文**に分割して特急メモ（`addItem(..., skipReviews=true)`）として保存（未ログイン中は保持しログイン後に保存）。android（MainActivity.kt / MainApplication.kt / AndroidManifest.xml）/ apps/mobile（App.tsx / services/sharedText.ts） |
| 2026-06-30 | （次回配信） | 追加：**グローバルホットキー `Ctrl+Alt+S` でどのアプリからでも特急メモを追加**（Discord 等の上に小窓を出す）。デスクトップ常駐アプリに `globalShortcut` を登録し、押すと枠なし・最前面・タスクバー非表示のクイック入力ポップアップ（既存の Web ルート `/clip` を流用＝メイン窓とログイン共有・特急メモに1件記録して自動で閉じる）を表示。**前面アプリの「選択中テキスト」をその場で取り込む**：ホットキー押下時に裏で `Ctrl+C` を合成送出（PowerShell `keybd_event` を `-EncodedCommand` で実行。先に Alt/Ctrl を keyup して押しっぱなし誤爆を防止）→ センチネル方式で選択有無を判定し、**選択あり＝その選択テキスト／選択なし＝既存クリップボード**を採用。**先頭の非空行＝タイトル／残り＝本文**に分割して `?title=&content=` で `/clip` へ渡す。Escape／✕で閉じる。トレイメニューにも「特急メモを追加 (Ctrl+Alt+S)」を追加。`/clip` はタイトル未入力時に本文1行目をタイトルへ流用するよう改善。electron/main.js（showQuickCapture / captureSelectionText / sendCopyKeystroke）/ apps/web/src/app/clip/page.tsx |
...
| 2026-06-26 | （次回配信） | 機能削除：**見出しアウトライン・インデント機能（`HeadingOutlineIndent`）を撤去**。「見出し直下の文章が右に字下げされて混乱のもと」になるため、ユーザー判断で機能まるごと削除（字下げ表示＋「見出し行頭で Backspace→見出し解除」の両方）。表示のみの拡張だったため**既存ページの本文は無傷**（字下げが消えるだけ・Backspaceは標準動作に戻る）。NotionEditor.tsx の import/extensions から除去し HeadingOutlineIndent.ts を削除。 |
| 2026-06-26 | （次回配信） | バグ修正：**「戻る」でスクロール位置が戻らないことが“たまに”ある**間欠問題を低減。原因＝復元の「離す基準」が絶対時間（到達後600ms/最長2.5s）だったため、本文枠が遅れて描画される重いページでは、枠出現直後の `onCreate` フォーカスやレイアウト揺れ（一瞬最上部へ飛ぶ）の前に復元が切り上がって負けることがあった。対策＝離す基準を**「本文枠が現れてからの経過」**に変更（到達しても枠出現から700ms粘り・最長3s）。ユーザー操作（ホイール/キー/タッチ）では即中断は従来どおり。notion-plus/[id]/page.tsx |
